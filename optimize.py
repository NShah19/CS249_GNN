...
def test_loss(model, loader, epoch):
    print('testing...........')
    model.eval()
    loss_all = 0
...
def objective(trial):
    # Generate the model.
    model_name = trial.suggest_categorical("model", ["NNGAT", "LI_NET"])
    pool_method = trial.suggest_categorical("poolmethod", ["topk", "sag"])
    ratio = trial.suggest_float("ratio", 0.1, 1)
    if model_name == 'NNGAT':
        model = NNGAT_Net(ratio, indim=opt.indim, poolmethod = pool_method).to(device)
    elif model_name == 'LI_NET':
            model = LI_Net(ratio, opt.indim).to(device)
    ...
    momentum = trial.suggest_float("momentum", 0.5, 1, log=True)
    weight_decay = trial.suggest_float("weightdecay", 0, 0.1, log=True)
    ...
    for epoch in range(0, opt.n_epochs):
        since  = time.time()
        tr_loss, s1_arr, s2_arr,le1,le2 = train(model, optimizer, scheduler, epoch)
        tr_acc = test_acc(model, train_loader)
        val_acc = test_acc(model, val_loader)
        val_loss = test_loss(model, val_loader, epoch)
